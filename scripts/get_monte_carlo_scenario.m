...
for x=1:num_ev_customers
    ...
    if sum(ismember(closed_delta_customers,tf_ID)) >=1
        ...
        modified_base_load(:,tf_ID_R) = num2cell(modified_base_load{:,tf_ID_R} + TFPID_flat_profiles_timetable{:,tf_ID});
        modified_base_load(:,tf_ID_B) = num2cell(modified_base_load{:,tf_ID_B} + TFPID_flat_profiles_timetable{:,tf_ID});
    else
        modified_base_load(:,tf_ID) = num2cell(modified_base_load{:,tf_ID} + TFPID_flat_profiles_timetable{:,tf_ID});
...
%% Validating at the feeder level
...
% grid on
% plot(modified_s_loads,"Time","S/kVA")
% hold off
